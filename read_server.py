...
def index():
    ...
    bufgps.seek(0)
    readings["lattitude"]=float(bufsens.readline())
    readings["longitude"]=float(bufsens.readline())
    readings["altitude"]=float(bufsens.readline())
    buftemp.seek(0)
    readings["temp"]=float(buftemp.readline())
    bufdist.seek(0)
    readings["distance"]=float(buftemp.readline())
    readings["speed"]=float(buftemp.readline())
